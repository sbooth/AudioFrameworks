...
static inline long try_sort_sync(cdrom_paranoia *p,
				 ...
				 long *offset,void (*callback)(long,int,void*),void *userdata){
  ...
  if(Bflags==NULL || (Bflags[post-cb(B)]&2)==0){
	...
	if(cv(B)[post-cb(B)]==iv(A)[zeropos]){
	  if(do_const_sync(B,A,Aflags,
	    ...
	    offset_add_value(p,&(p->stage1),*offset,callback,userdata);
...
static inline void stage1_matched(c_block *old,c_block *new,
				 ...
				 long matchoffset,void (*callback)(long,int,void*),void *userdata){
  long i;
  ...
  if(matchbegin-matchoffset<=cb(new) ||
    ...
    if(matchoffset)
      if(callback)(*callback)(matchbegin,PARANOIA_CB_FIXUP_EDGE,userdata);
  }else
    if(callback)(*callback)(matchbegin,PARANOIA_CB_FIXUP_ATOM,userdata);
  ...
  if(matchend-matchoffset>=ce(new) ||
    ...
    if(matchoffset)
      if(callback)(*callback)(matchend,PARANOIA_CB_FIXUP_EDGE,userdata);
  }else
    if(callback)(*callback)(matchend,PARANOIA_CB_FIXUP_ATOM,userdata);
...
static long i_iterate_stage1(cdrom_paranoia *p,c_block *old,c_block *new,
			     void(*callback)(long,int,void*),void *userdata){
  ...
  for(j=searchbegin;j<searchend;j+=23){
           ...
           callback because they will appear to be all skewed */
	{
	  ...
	  if(j<end){
	    stage1_matched(old,new,matchbegin,matchend,matchoffset,callback,userdata);
	  }else{
	    stage1_matched(old,new,matchbegin,matchend,matchoffset,NULL,NULL);
	  }
	}
	ret++;
...
static long i_stage1(cdrom_paranoia *p,c_block *new,
  ...
  while(ptr && ptr!=new){
    ...
    if(callback)(*callback)(cb(new),PARANOIA_CB_VERIFY,userdata);
    i_iterate_stage1(p,ptr,new,callback,userdata);
...
static long i_iterate_stage2(cdrom_paranoia *p,v_fragment *v,
			     sync_result *r,void(*callback)(long,int,void*),void *userdata){
  ...
  if(callback)(*callback)(fb(v),PARANOIA_CB_VERIFY,userdata);
...

  {
    ...
    for(j=searchbegin;j<searchend;j+=23){
      ...
      if(try_sort_sync(p,i,NULL,rc(root),j,
		       &matchbegin,&matchend,&offset,callback,userdata)){

	r->begin=matchbegin;
	r->end=matchend;
	r->offset=-offset;
	if(offset)if(callback)(*callback)(r->begin,PARANOIA_CB_FIXUP_EDGE,userdata);
	return(1);
      }
    }
  }
...
static long i_silence_match(root_block *root, v_fragment *v,
  ...
  if(begin<end){
    ...
    offset_add_value(p,&p->stage2,0,callback,userdata);
	...
	c_append(rc(root),vec+voff,fs(v)-voff);
      ...
      offset_add_value(p,&p->stage2,end-begin,callback,userdata);
    }else
      return(0);
  }
...
static long i_stage2_each(root_block *root, v_fragment *v,
  ...
  if(!rv(root)){
    ...
    if(i_iterate_stage2(p,v,&r,callback,userdata)){
	...
	if(matchA){
	  ...
	  if(matchA>0){
	    ...
	    if(callback)(*callback)(begin+rb(root)-1,PARANOIA_CB_FIXUP_DROPPED,userdata);
	    ...
	    if(callback)(*callback)(begin+rb(root)-1,PARANOIA_CB_FIXUP_DUPED,userdata);
	...
	if(matchA){
	  ...
	  if(matchA>0){
	    /* dropped bytes; add back from v */
	    if(callback)(*callback)(end+rb(root),PARANOIA_CB_FIXUP_DROPPED,userdata);
	    if(end+rb(root)<p->root.returnedlimit)
	      break;
	    c_insert(rc(root),end,cv(l)+endL,matchA);
	  }else{
	    /* duplicate bytes; drop from root */
	    if(callback)(*callback)(end+rb(root),PARANOIA_CB_FIXUP_DUPED,userdata);
	    if(end+rb(root)<p->root.returnedlimit)
	      break;
	...
	if(sizeB-offset>sizeA || v->lastsector){
	  ...
	  offset_add_value(p,&p->stage2,offset+vecbegin-rb(root),callback,userdata);
	}
      }
      if(l)i_cblock_destructor(l);
      free_v_fragment(v);
...
static int i_stage2(cdrom_paranoia *p,long beginword,long endword,
  ...
  while(flag){
	...
	if(first->one){
	  if(rv(root)==NULL){
	    if(i_init_root(&(p->root),first,beginword,callback,userdata)){
	      free_v_fragment(first);
	      flag=1;
	      ret++;
	    }
	  }else{
	    if(i_stage2_each(root,first,callback,userdata)){
	      ret++;
	      flag=1;
	    }
	  }
	...
	for(count=0;count<active;count++){
	  ...
	  if(first->one){
	    if(rv(root)!=NULL){
	      if(i_silence_match(root,first,callback,userdata)){
		ret++;
		flag=1;
	      }
	    }
...
static void verify_skip_case(cdrom_paranoia *p,void(*callback)(long,int,void*),void *userdata){
  ...
  if(callback)(*callback)(post,PARANOIA_CB_SKIP,userdata);
...
/* why do it this way?  We need to read lots of sectors to kludge
  ...
  while(sofar<totaltoread){
	...
	if(callback)(*callback)((adjread+thisread)*CD_FRAMEWORDS,PARANOIA_CB_READERR,userdata);
	...
	new->lastsector=-1;
      ...
      if(callback)(*callback)((adjread+secread-1)*CD_FRAMEWORDS,PARANOIA_CB_READ,userdata);
...
int16_t *paranoia_read(cdrom_paranoia *p, void(*callback)(long,int,void*),void *userdata){
  return paranoia_read_limited(p,callback,userdata,20);
}
...
int16_t *paranoia_read_limited(cdrom_paranoia *p, void(*callback)(long,int,void*),
			       void *userdata,int max_retries){
  ...
  while(rv(root)==NULL ||
	...
	i_stage2(p,beginword,
      ...
      i_end_case(p,endword+(MAX_SECTOR_OVERLAP*CD_FRAMEWORDS),
		 callback,userdata); /* only trips if we're already done */
    ...
    {
      c_block *new=i_read_c_block(p,beginword,endword,callback,userdata);

      if(new){
	if(p->enable&(PARANOIA_MODE_OVERLAP|PARANOIA_MODE_VERIFY)){

	  if(p->enable&PARANOIA_MODE_VERIFY)
	    i_stage1(p,new,callback,userdata);
	  else{
	...
	if(p->dynoverlap==MAX_SECTOR_OVERLAP*CD_FRAMEWORDS ||
	  ...
	  if(!(p->enable&PARANOIA_MODE_NEVERSKIP))verify_skip_case(p,callback,userdata);
	  retry_count=0;
	}else{

...
int shn_seek(shn_file *this_shn, unsigned int sample)
{
	if (NULL == this_shn)
		return -1;
	...
	if (this_shn->vars.seek_table_entries == NO_SEEK_TABLE)
	{
		shn_error(this_shn->config, "Cannot seek to %d because there is no seek information for this file.",sample);
		return -1;
	}

	if(sample > (unsigned int)(shn_get_frame_length(this_shn))) {
		shn_error(this_shn->config, "You cannot seek to this position! It is out of range!");
		return -1;
	}
	this_shn->vars.seek_to = sample;
	unsigned int ret = (sample/(this_shn->vars.seek_resolution))*this_shn->vars.seek_resolution;
...
static int get_wave_header(shn_file *this_shn)
{
    ...
    internal_ftype = UINT_GET(version, TYPESIZE, this_shn);
    ...
    nchan = UINT_GET(version, CHANSIZE, this_shn);

    /* get blocksize if version > 0 */
    if(version > 0)
    {
      int byte;
      blocksize = UINT_GET(version, (int) (log((double) DEFAULT_BLOCK_SIZE) / M_LN2),this_shn);
      maxnlpc = UINT_GET(version, LPCQSIZE, this_shn);
      nmean = UINT_GET(version, 0, this_shn);
      nskip = UINT_GET(version, NSKIPSIZE, this_shn);
      for(i = 0; i < nskip; i++)
      {
    ...
    while(1)
    {
      ...
      this_shn->vars.reading_function_code = 0;
        ...
        switch(cmd)
        {
          ...
          case FN_BLOCKSIZE:
            UINT_GET(version, (int) (log((double) blocksize) / M_LN2), this_shn);
            break;
...
static int shn_init_decode_state(shn_file *this_shn)
{
	...
	this_shn->status.version = FORMAT_VERSION;
	this_shn->status.bitshift = 0;
	this_shn->status.ftype = TYPE_EOF;
	this_shn->status.blocksize = DEFAULT_BLOCK_SIZE;
	this_shn->status.nchan = DEFAULT_NCHAN;
	this_shn->status.qlpc = NULL;
	this_shn->status.maxnlpc = DEFAULT_MAXNLPC;
	this_shn->status.nmean = UNDEFINED_UINT;
	this_shn->status.datarest = shn_get_frame_length(this_shn);
...
int shn_init_decoder(shn_file *this_shn) {
	...
	if(FORMAT_VERSION < 2) {
		...
		/* get version number */
		this_shn->status.version = getc_exit(this_shn->vars.fd);
	}
	else
#endif /* STRICT_FORMAT_COMPATABILITY */
	{
		...
		this_shn->status.version = MAX_VERSION + 1;
		while(this_shn->status.version > MAX_VERSION) {
			int byte = getc(this_shn->vars.fd);
			if(byte == EOF) {
	...
	/* check version number */
	if(this_shn->status.version > MAX_SUPPORTED_VERSION) {
		shn_error_fatal(this_shn,"Can't decode version %d", this_shn->status.version);
		return 0;
	}

	/* set up the default nmean, ignoring the command line state */
	this_shn->status.nmean = (this_shn->status.version < 2) ? DEFAULT_V0NMEAN : DEFAULT_V2NMEAN;
	...
	/* get the internal file type */
	this_shn->status.internal_ftype = UINT_GET(this_shn->status.version, TYPESIZE, this_shn);

	/* has the user requested a change in file type? */
	if(this_shn->status.internal_ftype != this_shn->status.ftype) {
		if(this_shn->status.ftype == TYPE_EOF)
			this_shn->status.ftype = this_shn->status.internal_ftype;    /*  no problems here */
		else             /* check that the requested conversion is valid */
			if(this_shn->status.internal_ftype == TYPE_AU1 || this_shn->status.internal_ftype == TYPE_AU2 || this_shn->status.internal_ftype == TYPE_AU3 || this_shn->status.ftype == TYPE_AU1 ||this_shn->status.ftype == TYPE_AU2 || this_shn->status.ftype == TYPE_AU3) {
				shn_error_fatal(this_shn,"Not able to perform requested output format conversion");
				return 0;
			}
	}

	this_shn->status.nchan = UINT_GET(this_shn->status.version, CHANSIZE, this_shn);

	/* get blocksize if version > 0 */
	int nskip = DEFAULT_NSKIP;
	if(this_shn->status.version > 0) {
		int byte;
		this_shn->status.blocksize = UINT_GET(this_shn->status.version, (int) (log((double) DEFAULT_BLOCK_SIZE) / M_LN2),this_shn);
		this_shn->status.maxnlpc = UINT_GET(this_shn->status.version, LPCQSIZE, this_shn);
		this_shn->status.nmean = UINT_GET(this_shn->status.version, 0, this_shn);
		nskip = UINT_GET(this_shn->status.version, NSKIPSIZE, this_shn);
		for(i = 0; i < nskip; i++) {
			byte = uvar_get(XBYTESIZE,this_shn);
		}
	}
	...
	this_shn->status.nwrap = MAX(NWRAP, this_shn->status.maxnlpc);

	/* grab some space for the input buffer */
	this_shn->status.buffer  = long2d((ulong) this_shn->status.nchan, (ulong) (this_shn->status.blocksize + this_shn->status.nwrap),this_shn);
	if (this_shn->vars.fatal_error)
		return 0;
	this_shn->status.offset  = long2d((ulong) this_shn->status.nchan, (ulong) MAX(1, this_shn->status.nmean),this_shn);
	if (this_shn->vars.fatal_error) {
		if (this_shn->status.buffer) {
			free(this_shn->status.buffer);
			this_shn->status.buffer = NULL;
		}
		return 0;
	}
	...
	int chan;
	for(chan = 0; chan < this_shn->status.nchan; chan++) {
		for(i = 0; i < this_shn->status.nwrap; i++)
			this_shn->status.buffer[chan][i] = 0;
		this_shn->status.buffer[chan] += this_shn->status.nwrap;
	}

	if(this_shn->status.maxnlpc > 0) {
		this_shn->status.qlpc = (int*) pmalloc((ulong) (this_shn->status.maxnlpc * sizeof(*this_shn->status.qlpc)),this_shn);
		if (this_shn->vars.fatal_error) {
			if (this_shn->status.buffer) {
				free(this_shn->status.buffer);
				this_shn->status.buffer = NULL;
			}
			if (this_shn->status.offset) {
				free(this_shn->status.offset);
				this_shn->status.buffer = NULL;
			}
			return 0;
		}
	...
	if(this_shn->status.version > 1)
		this_shn->status.lpcqoffset = V2LPCQOFFSET;

	init_offset(this_shn->status.offset, this_shn->status.nchan, MAX(1, this_shn->status.nmean), this_shn->status.internal_ftype);

	this_shn->vars.eof = FALSE;
	this_shn->status.chan = 0;
...
/* wind up */
	...
	if (this_shn->status.buffer) free((void *) this_shn->status.buffer);
	if (this_shn->status.offset) free((void *) this_shn->status.offset);
	if(this_shn->status.maxnlpc > 0 && this_shn->status.qlpc)
	free((void *) this_shn->status.qlpc);
...
unsigned int shn_get_frame_length(shn_file *this_shn) {
	if(this_shn) {
		if(this_shn->wave_header.length > 0)
			return (unsigned int)(this_shn->wave_header.data_size / ((this_shn->wave_header.bits_per_sample / 8) * this_shn->wave_header.channels));
	}
...
int shn_read(shn_file *this_shn, uchar *read_buffer, int bytes_to_read) {
	...
	while(!buffer_is_full) {
		int cmd = uvar_get(FNSIZE,this_shn);
		if (this_shn->vars.fatal_error)
			goto cleanup;
		...
		switch(cmd) {
			...
			case FN_QLPC:
				{
					slong coffset, *cbuffer = this_shn->status.buffer[this_shn->status.chan];
					int resn = 0, nlpc, j;
					...
					if(this_shn->status.nmean == 0)
						coffset = this_shn->status.offset[this_shn->status.chan][0];
					else
					{
						slong sum = (this_shn->status.version < 2) ? 0 : this_shn->status.nmean / 2;
						for(i = 0; i < this_shn->status.nmean; i++)
							sum += this_shn->status.offset[this_shn->status.chan][i];
						if(this_shn->status.version < 2)
							coffset = sum / this_shn->status.nmean;
						else
							coffset = ROUNDEDSHIFTDOWN(sum / this_shn->status.nmean, this_shn->status.bitshift);
					}
					...
					if(this_shn->status.nmean > 0) {
						...
						if(this_shn->status.version < 2)
							this_shn->status.offset[this_shn->status.chan][this_shn->status.nmean - 1] = sum / this_shn->status.blocksize;
						else
					...
					if(this_shn->status.chan == this_shn->status.nchan - 1) {
						...
						if (this_shn->vars.seek_to != -1) {
							...
							this_shn->status.offset[0][0]  = shn_uchar_to_slong_le(seek_info->data+48);
							this_shn->status.offset[0][1]  = shn_uchar_to_slong_le(seek_info->data+52);
							this_shn->status.offset[0][2]  = shn_uchar_to_slong_le(seek_info->data+56);
							this_shn->status.offset[0][3]  = shn_uchar_to_slong_le(seek_info->data+60);
							if (this_shn->status.nchan > 1) {
								this_shn->status.buffer[1][-1] = shn_uchar_to_slong_le(seek_info->data+36);
								this_shn->status.buffer[1][-2] = shn_uchar_to_slong_le(seek_info->data+40);
								this_shn->status.buffer[1][-3] = shn_uchar_to_slong_le(seek_info->data+44);
								this_shn->status.offset[1][0]  = shn_uchar_to_slong_le(seek_info->data+64);
								this_shn->status.offset[1][1]  = shn_uchar_to_slong_le(seek_info->data+68);
					...
					this_shn->status.chan = (this_shn->status.chan + 1) % this_shn->status.nchan;
					break;
				}
				break;
